...
{{-- ✅ GOOD - Escaped output --}}
<div>{{ $userInput }}</div>  {{-- Blade auto-escapes --}}
...
Route::middleware('auth')->get('/admin/users', [UserController::class, 'index']);

// ✅ BETTER - Authentication and authorization
Route::middleware(['auth', 'can:view-users'])->get('/admin/users', [UserController::class, 'index']);
```
...
```env
# ❌ BAD - Missing or default key
APP_KEY=

# ❌ BAD - Placeholder key
APP_KEY=base64:your-key-here

# ✅ GOOD - Secure generated key
APP_KEY=base64:randomly-generated-32-byte-key-here
```
...
```bash
# ❌ BAD - Vulnerable package installed
composer require vulnerable/package:1.0.0

# ✅ GOOD - Check for vulnerabilities
composer audit
composer require safe/package:2.0.0
```
...
- [View all Security Analyzers](#all-analyzers)
